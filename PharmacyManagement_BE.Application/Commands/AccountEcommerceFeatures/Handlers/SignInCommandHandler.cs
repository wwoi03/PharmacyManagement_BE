        ...
        public async Task<ResponseAPI<SignInDTO>> Handle(SignInCommandRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (user == null || !await _userManager.CheckPasswordAsync(user, request.Password))
                    return new ResponseSuccessAPI<SignInDTO>(StatusCodes.Status401Unauthorized, "Thông tin tài khoản hoặc mật khẩu không chính xác.");

                // Kiểm tra xem tài khoản có bị khóa không
                if (await _userManager.IsLockedOutAsync(user))
                    return new ResponseSuccessAPI<SignInDTO>(StatusCodes.Status403Forbidden, $"Tài khoản của bạn đã bị khóa đến ngày {user.LockoutEnd}.");
