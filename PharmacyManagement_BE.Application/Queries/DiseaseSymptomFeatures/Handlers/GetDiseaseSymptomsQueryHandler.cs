...
{
    internal class GetDiseaseSymptomsQueryHandler : IRequestHandler<GetDiseaseSymptomsQueryRequest, ResponseAPI<List<DiseaseSymptomDTO>>>
    {
        private readonly IPMEntities _entities;
        private readonly IMapper _mapper;

        public GetDiseaseSymptomsQueryHandler(IPMEntities entities, IMapper mapper)
        {
            this._entities = entities;
            this._mapper = mapper;
        }

        public async Task<ResponseAPI<List<DiseaseSymptomDTO>>> Handle(GetDiseaseSymptomsQueryRequest request, CancellationToken cancellationToken)
        {
            try
            {
                //Lấy danh sách bệnh
                var list = await _entities.DiseaseSymptomService.GetAllByDisease(request.Id);
