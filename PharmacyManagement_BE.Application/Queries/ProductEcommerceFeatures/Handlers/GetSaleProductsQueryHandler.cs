...
{
    internal class GetSaleProductsQueryHandler : IRequestHandler<GetSaleProductsQueryRequest, ResponseAPI<List<ItemProductDTO>>>
    {
        private readonly IPMEntities _entities;

        public GetSaleProductsQueryHandler(IPMEntities entities)
        {
            this._entities = entities;
        }

        public async Task<ResponseAPI<List<ItemProductDTO>>> Handle(GetSaleProductsQueryRequest request, CancellationToken cancellationToken)
        {
            try
            {
                var currentDate = DateTime.Now;
                var response = await _entities.ProductService.GetSaleProducts();
