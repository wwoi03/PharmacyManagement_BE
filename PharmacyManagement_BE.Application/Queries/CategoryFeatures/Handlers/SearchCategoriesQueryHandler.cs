...
{
    internal class SearchCategoriesQueryHandler : IRequestHandler<SearchCategoriesQueryRequest, ResponseAPI<List<ListCategoryDTO>>>
    {
        private readonly IPMEntities _entities;

        public SearchCategoriesQueryHandler(IPMEntities entities)
        {
            this._entities = entities;
        }

        public async Task<ResponseAPI<List<ListCategoryDTO>>> Handle(SearchCategoriesQueryRequest request, CancellationToken cancellationToken)
        {
            try
            {
                var response = await _entities.CategoryService.SearchCategories(request.ContentStr);
