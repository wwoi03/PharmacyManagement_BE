...
{
    internal class SearchStaffQueryHandler : IRequestHandler<SearchStaffQueryRequest, ResponseAPI<List<StaffResponse>>>
    {
        ...
        public SearchStaffQueryHandler(IPMEntities entities, IMapper mapper, UserManager<ApplicationUser> userManager, RoleManager<IdentityRole<Guid>> roleManager)
        {
        ...
        public async Task<ResponseAPI<List<StaffResponse>>> Handle(SearchStaffQueryRequest request, CancellationToken cancellationToken)
        {
            try
            {
                // Lấy danh sách nhân viên của chi nhánh
                var staffs = await _entities.StaffService.SearchStaffs(request.SearchString);
